use ethers::{abi::{decode, Address}, contract::abigen, middleware::SignerMiddleware, providers::{Http, Middleware, Provider}, signers::{LocalWallet, Signer}, types::{BlockNumber, Filter, TransactionRequest, U256, U64}, utils::parse_ether};
use secp256k1::SecretKey;
use std::{error::Error, sync::Arc};
use crate::{on_chain::utils::{read_meta_data, write_meta_data}, recipient::recipient::{scan, RecipientInputData}, sender::sender::send, versions::v1::calculate_stealth_priv_key};
...
pub const PROVIDER_STRING: &str = "";
pub const CHAIN_ID: u64 = 11155111;
pub const CONTRACT_ADDRESS: &str = "0xED737936e54fAC3B4fF271928a7E31Bc140119B0";
//pub const CONTRACT_ADDRESS: &str = "0xA0670138449C6E0e9EB2116949a783A9b0D5A22A";
...
impl BlockchainService {
    ...
    pub async fn send_eth(value: f64, wallet: &String, meta_address: &String) -> Result<Address, Box<dyn Error>>{
        let (stealth_address, ephemeral_pub_key, view_tag) = send(&meta_address);

        let provider = Provider::<Http>::try_from(PROVIDER_STRING)?;
        
        let wallet = wallet.parse::<LocalWallet>()?.with_chain_id(CHAIN_ID); 
        
        let client = Arc::new(SignerMiddleware::new(provider, wallet)); 

        let contract_address:Address = CONTRACT_ADDRESS.parse()?;
        let contract = PQSAP_Announcer::new(contract_address, client.clone()); 

        let value = U256::from(parse_ether(value)?);

        let _ = contract.send_eth_via_proxy(stealth_address, ephemeral_pub_key.clone().into(), view_tag.clone().into())
        .value(value).send().await?.await?;
    ...
    pub async fn receive_eth(k_hex: &String, v_hex: &String, destination_wallet: &String) -> Result<Vec<(Address, U256)>, Box<dyn Error>>{  

        let k_bytes = hex::decode(k_hex.clone()).expect("Failed to decode hex");
        let k = SecretKey::from_slice(&k_bytes).unwrap();
    
        // find all new transactions
        let (stealth_addresses, ephemeral_pub_key_reg, view_tags) = fetch_transactions().await?; 
        ...
        let mut result: Vec<(Address, U256)> = vec![]; 
        ...
        if shared_secrets.len() > 0{
            for (i, s) in shared_secrets.iter().enumerate(){
                ...
                let wallet = p.parse::<LocalWallet>()?.with_chain_id(CHAIN_ID);
                ...
                if balance > gas_cost{
                    ...
                    result.push((stealth_addresses[i], max_amount))
                }else{
                    result.push((stealth_addresses[i], U256::from(0)));
                }
...
async fn fetch_transactions() -> Result<(Vec<Address>, Vec<String>, Vec<String>), Box<dyn Error>>{
    let provider = Provider::<Http>::try_from(PROVIDER_STRING)?; 
    let contract_address:Address = CONTRACT_ADDRESS.parse()?; 
    
    // Read latest block accessed 
    let latest_block = read_meta_data();
    ...
    let _ = write_meta_data(logs.last().unwrap().block_number.unwrap().as_u64());
...
pub async fn fetch_meta_address(recipient: &str) -> Result<String, Box<dyn Error>>{
